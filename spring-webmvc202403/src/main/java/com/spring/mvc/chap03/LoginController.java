    ...
    public String loginCheck(String id, String pw, Model model) {
       ...
       /* 방법1
        if ("grape111".equals(id) && "ggg9999".equals(pw)) {
            model.addAttribute("success", true); // 로그인 성공
            model.addAttribute("id", id); // 성공한 아이디 전달
        } else if ("grape111".equals(id)) {
            model.addAttribute("success", false); // 로그인 실패(비밀번호 오류)
            model.addAttribute("id", id); // 입력한 아이디 전달
        } else {
            model.addAttribute("success", false); // 로그인 실패(아이디 존재하지 않음)
        }
